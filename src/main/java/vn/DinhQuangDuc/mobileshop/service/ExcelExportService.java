...
public class ExcelExportService {
    ...
    public void exportUsers(HttpServletResponse response, List<User> users) throws Exception {
        try (XSSFWorkbook workbook = new XSSFWorkbook()) {
            ...
            // Khớp với các trường có trong User.java
            ...
            for (User u : users) {
                ...
                createCell(row, 7, u.getDateOfBirth(), centerStyle); // Kiểu String trong User.java
    ...
    public void exportProducts(HttpServletResponse response, List<Product> products) throws Exception {
        try (XSSFWorkbook workbook = new XSSFWorkbook()) {
            ...
            // Khớp với các trường có trong Product.java
            String[] headers = {
                    "ID", "Tên sản phẩm", "Giá (VNĐ)", "Số lượng", "Đã bán",
                    "Hãng (Factory)", "Đối tượng", "Hệ điều hành", "CPU", "RAM", "ROM",
    ...
    // ==========================================
    // 3. MODULE: QUẢN LÝ ĐƠN HÀNG (Master-Detail)
    // ==========================================
    public void exportOrders(HttpServletResponse response, List<Order> orders) throws Exception {
        try (XSSFWorkbook workbook = new XSSFWorkbook()) {
            ...
            // Khớp với các trường có trong Order.java
            String[] headers = { "Mã ĐH", "Khách hàng", "SĐT", "Tài khoản (User)", "Địa chỉ nhận", "Tổng tiền",
                    "Trạng thái", "Ngày đặt", "Ngày giao", "Ngày nhận dự kiến", "Ngày nhận", "Ngày hủy", "Ghi chú/SP" };
            createHeaderRow(workbook, sheet, headers, 4);
            ...
            // Style cho dòng sản phẩm chi tiết (Nền xám, nghiêng)
            ...
            for (Order o : orders) {
                ...
                // DÒNG CHI TIẾT TỪNG SẢN PHẨM TRONG ĐƠN (Trải xuống dưới)
                if (o.getOrderDetails() != null && !o.getOrderDetails().isEmpty()) {
                    for (OrderDetail detail : o.getOrderDetails()) {
                        ...
                        // Merge cột 0 -> 3 để thụt lề cho đẹp
                        ...
                        // Fill viền cho các cột còn lại để bảng vuông vắn
            ...
            // TỔNG KẾT BÁO CÁO (Nằm ở cuối file)
            ...
            Row sRow3 = sheet.createRow(rowIdx + 3);
            createSummaryCell(workbook, sRow3, 0, 4, "ĐƠN THÀNH CÔNG:", successOrders + " đơn");

            Row sRow4 = sheet.createRow(rowIdx + 4);
            createSummaryCell(workbook, sRow4, 0, 4, "ĐƠN ĐÃ HỦY:", cancelledOrders + " đơn");
    ...
    // ==========================================
    // 4. CORE STYLES & UTILITIES (DÙNG CHUNG)
    // ==========================================
    ...
    private void createSummaryCell(XSSFWorkbook wb, Row row, int startCol, int endCol, String label, String value) {
        // Dùng row.getSheet() thay vì sheet
    ...
    private void finalizeSheet(XSSFSheet sheet, int colCount) {
        for (int i = 0; i < colCount; i++) {
            ...
            sheet.setColumnWidth(i, sheet.getColumnWidth(i) + 1200); // Thêm khoảng trắng cho thoáng
        }
    }
